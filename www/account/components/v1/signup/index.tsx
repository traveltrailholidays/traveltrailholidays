...
const Signup = () => {
  ...
  const handleEmailCheck = async () => {
    ...
    try {
      ...
      const sendMailResposne = await API.post('/auth/generate-signup-otp', { email });
      if (sendMailResposne.data.code !== 200) {
        toast.error(response.data.message);
        return;
      };
      localStorage.setItem('_tthAuthSiginEmail', email);
      router.push('/v1/signup/steps/verify-email')
    } catch (error) {
    ...
    <AuthLayout>
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleEmailCheck(); }}>
        ...
        <div className='pt-12 flex justify-between items-center'>
          <UrlNavigation href={'/v1/signin'} className='text-indigo-400 hover:text-indigo-500 dark:text-indigo-200 hover:dark:text-indigo-300 font-medium'>
            Signin
          </UrlNavigation>
          <button
            type="submit"
